...
namespace VoiceAndroidApp
{
    class MagmaColorMap
    {
        private static readonly float[,] _data = new float[,]
        {
        ...
        private static readonly int _length = _data.GetLength(0);
        private static readonly int[] _colors;

        static MagmaColorMap()
        {            
            _colors = new int[_length];
            for (int idx = 0; idx < _length; idx++)
            {
                _colors[idx] = Color.Rgb(
                    (int)(255 * _data[idx, 0]),
                    (int)(255 * _data[idx, 1]),
                    (int)(255 * _data[idx, 2]));
            }
        ...
        public static int Rgb(float v)
        {
            int idx = Math.Clamp((int)(v * _length), 0, _length - 1);
            return _colors[idx];
        }
    }
}